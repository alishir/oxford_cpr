...
all() ->
  ...
  %  {group, single_bidder_integration}].
...
groups() -> 
  [{auction_data_dep_unit, [], [test_start_link,
   ...
   {auction_data_dep_unit, [], [test_init,
                                test_auction_ended,
                                test_check_for_invalid_bid,
                                test_check_leading_bid,
                                test_get_next_itemid]}].
  %  {single_bidder_integration, [], [test_bid_single_bidder]}].
  ...
  [{state_map, M} | 
    [{auction, AuctionId} | 
      [{itemids, [ItemId1, ItemId2]} | 
        Config]]].
% init_per_group(single_bidder_integration, Config) ->
...
end_per_group(statem_dep, _Config) ->
  ok.
% end_per_group(single_bidder_integration, Config) ->
